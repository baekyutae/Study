...
#### \+추천 시스템에서의 Pagination 과 대안 전략

- **추천 시스템의 유즈케이스 (무한 스크롤)**:
    
    - 유튜브 홈 피드나 인스타그램 피드처럼 사용자가 끊임없이 콘텐츠를 탐색하는 환경에서는 스크롤을 내릴 때마다 10~20개의 데이터를 지연 없이 부드럽게 제공해야 함.
        
    - **가장 큰 난관 (Data Drift)**: 추천 로직은 사용자의 실시간 클릭, 좋아요 등에 따라 순위가 계속 변동됨. 만약 여기서 **오프셋(Offset) 방식**을 쓰게 되면, 사용자가 1페이지를 보는 동안 DB의 순위가 바뀌어 2페이지 요청 시 이미 본 콘텐츠가 또 나오거나(중복), 봐야 할 콘텐츠가 뒤로 밀려버리는(누락) 치명적인 문제가 발생함.
        
    - **해결책**: 따라서 기준점이 명확한 **커서(Cursor) 기반 페이징**을 도입하여 중간에 데이터가 추가/삭제되더라도 일관된 위치부터 데이터를 조회할 수 있도록 해야 함.
        
- **하지만 커서(Cursor) 방식도 완벽하지 않음**:
    
    - "최신순"처럼 단순한 정렬이 아니라, 추천 시스템처럼 "사용자 취향 점수 70% + 최신순 30%" 같이 복잡하게 계산된 점수로 정렬될 경우, 고유한 커서 기준점을 잡기가 매우 까다로워지고 백엔드 연산 부하가 커짐.
        
    - 이를 해결하기 위해 페이징 자체를 다른 방식으로 우회하는 대안 아키텍처를 고려할 수 있음.
        

#### 대안 1: 세션 기반 스냅샷 (Session-based Snapshotting / Caching)

- **동작 원리**: 사용자가 앱을 켜거나 추천 탭에 처음 진입하는 순간, 무거운 추천 AI 모델이 100~200개의 추천 리스트를 한 번에 생성하여 **Redis 같은 인메모리 캐시(메모리)에 '스냅샷'처럼 얼려두고(저장하고)** 제공하는 방식임.
    
- **장점**:
    
    - **압도적인 성능과 일관성**: 캐시에서 데이터를 뚝뚝 잘라서(Slicing) 주기만 하면 되므로, 스크롤 시 응답 속도가 매우 빠름. 보는 동안 순위가 바뀔 일이 아예 없으므로 중복/누락(Data Drift) 문제가 0%로 차단됨.
        
    - **백엔드 연산 보호**: 아주 무거운 AI 추론 연산이나 복잡한 DB 쿼리를 매 스크롤마다 하지 않고 처음에 딱 한 번만 수행하여 서버 자원을 아낄 수 있음.
        
- **단점**:
    
    - **높은 인프라 비용**: 동시 접속자 수만큼 추천 리스트를 모두 메모리에 들고 있어야 하므로 RAM 서버 유지 비용이 비쌈.
        
    - **데이터의 낡음(Staleness)**: 유저가 새로고침을 하지 않고 오랫동안 스크롤만 내린다면, 실시간 데이터가 반영되지 않은 과거의 데이터만 계속 보게 됨.
        

#### 대안 2: 클라이언트 사이드 페이징 (Client-side Pagination)

- **동작 원리**: 서버가 처음에 50개~100개의 큰 데이터 덩어리를 통째로 프론트엔드(웹/앱)로 한 번에 던져줌. 이후 스크롤을 내릴 때는 브라우저가 자신의 메모리에 있는 데이터를 알아서 10개씩 화면에 렌더링함.
    
- **장점**:
    
    - **추가 서버 요청 Zero**: 스크롤을 내릴 때 백엔드 서버로 오는 API 요청이 아예 없으므로 서버 부하를 크게 줄일 수 있음.
        
- **단점**:
    
    - **자원 낭비**: 사용자가 스크롤을 조금만 내리고 앱을 끄면, 사용하지도 않을 나머지 데이터를 보내느라 백엔드의 네트워크 대역폭과 자원을 크게 낭비한 셈이 됨. 방대한 데이터를 다루는 환경에서는 클라이언트 기기의 메모리를 과도하게 잡아먹어 앱이 종료될 위험이 있어 추천 시스템에서는 잘 쓰이지 않음.